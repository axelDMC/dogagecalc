...
function computeDeductions(
  ...
  return DEDUCTIONS[category].filter((d) => {
    ...
    return Object.entries(d.requiredAnswers).every(
      ([key, val]) => answers[key as QuestionId] === val
    );
  });
}
...
export default function TaxChecklistTool() {
  ...
  useEffect(() => {
    ...
    return () => {
      const el = document.getElementById("tax-print-styles");
      if (el) el.remove();
    };
  }, []);
  ...
  function handleGenerate() {
    if (!allAnswered || !category) return;
    setCheckedItems({});
    setStep("results");
  }
      ...
      <div>
        ...
        <button
          ...
          style={{
            ...
            background: allAnswered ? "#2563eb" : "rgba(37,99,235,0.4)",
            color: "#fff",
            border: "none",
  ...
  return (
    <div id="tax-checklist-print">
      <div className="print-only" style={{ marginBottom: "16px" }}>
        <strong>FreelanceTaxChecklist.com — Freelance Tax Deduction Checklist</strong>
        <br />
        <span style={{ fontSize: "13px" }}>
          Category: {category} | Generated: {new Date().toLocaleDateString()}
        </span>
      </div>
      <div
        style={{
      ...
      >
        ...
        >
          Est. savings: {formatMoney(totalMin)} – {formatMoney(totalMax)}
        </span>
      </div>
            ...
            >
              <button
                ...
                style={{
                  ...
                  minWidth: "20px",
                  borderRadius: "6px",
                  border: checked ? "1px solid #2563eb" : "1px solid var(--border)",
              ...
              >
                {formatMoney(d.savingsMin)} – {formatMoney(d.savingsMax)}
              </div>
            </div>
          );
        })}
      ...
      >
        Estimates are for informational purposes only and do not constitute tax advice.
